...
% thickline - provides heavier lines and arrows
...
\hypersetup{pdftitle={Smart Solar Panel System Voltage},
...
%    \clearpage
    ...
    \lhead{Smart Solar Panel System}
    % Lab report title
    \rhead{Voltage Plot \& Table}
...
%               max space between ticks=1,
                ...
                xlabel=Time Unit,
                ylabel=Voltage (mV),]
                \addplot[color=red, mark=*, mark size=0.4, style=thick]table[x=Time, y=Voltage, col sep=comma]{solar.csv};
            \end{axis}
        \end{tikzpicture}
    \captionof{figure}{Current Plot}
    \label{fig:vPlot}
    \end{figure}
    \clearpage
%    \section{Data Table}
...
%    \begin{table}[htbp]
%        \centering
%        \caption{Data collected using solar panel}
%        \csvautotabular{solar.csv}
%        \label{tab:data}
%    \end{table}
